...
County Health Rankings and Roadmaps (CHRR) data was downloaded from the years 2017-2022 (exluding the missing 2020 year). Data from CHRR originally came in two separate CSVs for each year and broke up the data by each county in the United States. 
...
Second, we joined the two CSVs for each year to have one CSV per year and added a "Year" column to help with merging later. Then we found the mean for each column for each state.
