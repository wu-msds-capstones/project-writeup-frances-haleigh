...
In order to evaluate the health status of a group of people, we turned to three common metrics for doing so: life expectancy, mortality, and infant mortality.[⁸](#ref8) In the following sections, we modeled each of these by a list of variables representing access to care (see @sec-s_table_2) and ensured there was not multicollinearity between the remaining variables and that model assumptions were met **(ADD REFERENCE TO FIGURES HERE)**. Our goal was to determine how access to care impacted each of these three health metrics.
...
This suggests that lower access to care (`PCP Ratio` and `Percentage of Broadband Internet Access`) and the most segregated areas (fourth quartile of `Segregation Index`) have lower life expectancies. 
:::
...
### Infant Mortality - left off here
...
![Figure 13: caption](./Figures/kbest_inf_mor.png)
