...
In order to evaluate the health status of a group of people, we turned to three common metrics for doing so: life expectancy, mortality, and infant mortality.[⁸](#ref8) In the following sections, we modeled each of these by a list of variables representing access to care (see @sec-s_table_2) and ensured there was not multicollinearity between the remaining variables and that model assumptions were met (see @sec-s_figure_s.4a). Our goal was to determine how access to care impacted each of these three health metrics.
...
Additional Kaplan-Meier Curves for life expectancy were made for the following variables: `PCP Ratio`, `Segregation Index`, and `Percentage of Broadband Internet Access`. Log rank tests were performed for each, and all of the overall results were significant (not shown here). There were significant differences between each quartile in the `PCP Ratio` and `Percentage of Broadband Internet Access` plots, but the fourth quartile from the `Segregation Index` was the only one that was significantly different from the rest. <a href="./images/KM_PCPratio.png" title="Figure 9">Figure 9</a> shows each of the KM curves. These results suggest that lower access to care (`PCP Ratio` and `Percentage of Broadband Internet Access`) and the most segregated areas (fourth quartile of `Segregation Index`) have lower life expectancies.

::: {layout-ncol=3}
![Figure 9a: Having less PCPs available in a community reduces life expectancy.](./images/KM_PCPratio.png){group="km_gallery"}

![Figure 9b: Living in the most segregated communities (ratio of Blacks versus Whites) reduces life expectancy.](./images/KM_Segregation.png){group="km_gallery"}

![Figure 9c: Having reduced access to internet reduces life expectancy.](./images/KM_broadband.png){group="km_gallery"}
 
:::

We then used machine learning algorithms to predict life expectancy of individuals based on these significant covariates along with the top ten features found through KBest (see <a href="./Figures/kbest_life_exp.png" title="Figure 10">Figure 10</a>). It is important to note that a lot of features were found in both the linear regression model explained above and in the KBest result. 

![Figure 10: Top 10 features in the life expectancy dataset. Note that a lot of features were found in both the linear regression model explained above and in this result. Income, food insecurity, and internet access seem to be most associated with differences in life expectancy.](./Figures/kbest_life_exp.png)
